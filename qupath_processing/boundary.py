...
def rotated_cells_from_top_line(top_left, top_right, cells_dataframe):
    ...
    (
        cells_dataframe["Centroid X µm"],
        cells_dataframe["Centroid Y µm"],
    ) = rotate_points_list(
        cells_dataframe["Centroid X µm"], cells_dataframe["Centroid Y µm"], theta
    )
...
def compute_dbscan_eps(cell_position_file_path, layers_name, factor=4):
    ...
    for layer_name in layers_name:
        cells_mean_delaunay = (
            df[df["Class"] == layer_name]["Delaunay: Mean distance"]
...
def add_border_flag(cells_dataframe, layers_name, distance=20):
    ...
    for index, row in cells_dataframe.iterrows():
        if row.Class != "Layer 6 b":
            next_layer_dfs = cells_dataframe[
                cells_dataframe.Class.isin(next_layers_dict[row.Class])
            ]
            dfy = next_layer_dfs[
                abs(next_layer_dfs["Centroid Y µm"] - row["Centroid Y µm"]) < distance
            ]
