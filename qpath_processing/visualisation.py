...
def plot_split_polygons_and_cell_depth(split_polygons, s1_coordinates, cells_centroid_x, cells_centroid_y):
    """
    ...
    cells_depth = compute_cells_depth(split_polygons, cells_centroid_x, cells_centroid_y)
    fig = plt.figure()
    ...
    plt.scatter(cells_centroid_x, cells_centroid_y, c=np.array(cells_depth) / 100, s=1)
    plt.show()
